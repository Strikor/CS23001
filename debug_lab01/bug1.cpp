...
void UniqRandInt(int max, int n, int result[]) {
    ...
    while(cntr < n) {
        ...
        if (!(inArray(result, cntr, r))) {
            //std::cerr << "added r to arr" << std::endl;
            result[cntr] = r;
            cntr++;
        }
        
    }
    //std::cerr << "UniqRand complete" << std::endl;
    return;
...
bool inArray(int array[], int arrSize, int x) {
    ...
    for (int i = 0; i < arrSize; ++i) {
        //std::cerr << "if statement ran" << std::endl;
        if (array[i] == x) {
            return true;
        }
    }

    //std::cerr << "inArray complete" << std::endl;
    return false;
